...
static int init_input_d(AVFormatContext *s, const char *filename,
                      AVDictionary **options, int offset)
{
    int ret;
    AVProbeData pd = { filename, NULL, 0 };
    int score = AVPROBE_SCORE_RETRY;
    ...
    if ((ret = avio_open3(&s->pb, filename, AVIO_FLAG_READ | s->avio_flags,
                          &s->interrupt_callback, options, offset)) < 0)
        return ret;
    if (s->iformat)
        return 0;
    return av_probe_input_buffer2(s->pb, &s->iformat, filename,
...
int avformat_open_input_d(AVFormatContext **ps, const char *filename,
                        AVInputFormat *fmt, AVDictionary **options, int offset)
{
    AVFormatContext *s = *ps;
    int ret = 0;
    AVDictionary *tmp = NULL;
